...
describe("Swag lab conditional purchase journey", () => {
    it("Should logn in successfully", async()=>{
        await loginActions.enterUserName('performance_glitch_user');
        await loginActions.enterPassword('secret_sauce');
        await loginActions.clickOnLogInButton();
    });

    it("Should successfully add single product in the cart", async () =>{
        ...
        await addToCartActions.filterByLetter('Name (Z to A)');
        await browser.pause(1000);
        await addToCartActions.clickOnFirstItemAddButton();
